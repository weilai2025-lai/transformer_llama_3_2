...
假設字典（`sorted_vocab`）中有以下 token 與分數：

| id | token | vocab_scores[id] |
|----|-------|------------------|
| 10 | `"fp"` | 5.0 |
| 11 | `"pg"` | 3.0 |
| 12 | `"ga"` | 4.0 |
...
#### Step 0：查 sorted_vocab

輸入 `"fpgaaaaaa"` → 直接查 sorted_vocab → **找不到** ❌  
→ 進入 BPE 流程
...
檢查所有相鄰組合，去 `sorted_vocab` 查，並比較分數：
...
```c
// 1. 把兩個相鄰 token 組合成字串
sprintf(merged, "%s%s", tokens[i], tokens[i+1]);

// 2. 去 sorted_vocab 做二分搜尋
int id = str_lookup(merged, sorted_vocab, vocab_size);
...
> ⚠️ **注意：**  
> 合併順序必須和訓練時一致（都是選最高分的），否則會產生不同的 token 序列，模型推論結果會出錯。
