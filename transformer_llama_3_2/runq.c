/* Inference for Llama-2 Transformer model in pure C, int8 quantized forward
 * pass. */
...
char *decode(Tokenizer *t, int prev_token, int token) {
  ...
  // following BOS (128000) token, sentencepiece decoder strips any leading
  // whitespace (see PR #89)
  if (prev_token == 128000 && piece[0] == ' ') {
    piece++;
  }
...
void encode(Tokenizer *t, char *text, int8_t bos, int8_t eos, int *tokens,
  ...
  *n_tokens = 0;
  ...
  // regex to force splits from the Meta Llama 3 repo
  // (https://github.com/meta-llama/llama3/blob/main/llama/tokenizer.py):
  const char *normal_regex =
      "(?i:'s|'t|'re|'ve|'m|'ll|'d)|[^\r\n\\p{L}\\p{N}]?\\p{L}+|\\p{N}{1,3}| "
      "?[^\\s\\p{L}\\p{N}]+[\r\n]*|\\s*[\r\n]+|\\s+(?!\\S)|\\s+";
  // regex for special tokens ex. <|eot_id|>, <|start_header_id|> etc
  // in the tiktoken code its just all of the specail tokens "or"ed together
  // (https://github.com/openai/tiktoken/blob/main/src/lib.rs#L441)
  // TODO: add this in tokenizer export
  const char *special_regex =
      "<\\|begin_of_text\\|>|<\\|end_of_text\\|>|<\\|start_header_id\\|>|<\\|"
      "end_header_id\\|>|<\\|eot_id\\|>|<\\|reserved_special_token_([0-9]|[1-9]"
      "[0-9]|1[0-9][0-9]|2[0-4][0-9]|250)\\|>";

  const char *error;
  int erroffset;
  pcre *re_special = pcre_compile(special_regex, 0, &error, &erroffset, NULL);
  pcre *re_normal =
      pcre_compile(normal_regex, PCRE_UTF8 | PCRE_UCP | PCRE_CASELESS, &error,
                   &erroffset, NULL);
  ...
  // inspired from the tiktoken code:
  // https://github.com/openai/tiktoken/blob/main/src/lib.rs#L216
  while (idx < text_len) {
    int ovector[512];
    int special_match = pcre_exec(re_special, NULL, text + idx, text_len - idx,
                                  0, 0, ovector, 512);
    int special_match_len = ovector[1] - ovector[0];

    size_t end = (special_match >= 0) ? idx + ovector[0] : text_len;

    // proces the tokens up to the start of the next special token, or end if
    // there is none
    while (idx < end) {
      int normal_match =
          pcre_exec(re_normal, NULL, text + idx, end - idx, 0, 0, ovector, 512);
      if (normal_match < 0)
        break;
      ...
      if (token_id != -1) {
        ...
        // 10xxxxxx process the raw (UTF-8) byte sequence of the input string
        size_t str_len = 0;
        for (char *c = sequence; *c != '\0'; c++) {
          ...
          // with "10" in first two bits so in English this is: "if this byte is
        ...
        while (1) {
          ...
          for (int j = 0; j < split_n_tokens - 1; j++) {
            ...
            sprintf(str_buffer, "%s%s", t->vocab[split_tokens[j]],
                    t->vocab[split_tokens[j + 1]]);
            int id = str_lookup(str_buffer, t->sorted_vocab, t->vocab_size);
          ...
          // delete token at position best_idx+1, shift the entire sequence back
          // 1
    ...
    if (special_match >= 0) {
      ...
      int special_token_id =
          str_lookup(matched_str, t->sorted_vocab, t->vocab_size);
...
void chat(Transformer *transformer, Tokenizer *tokenizer, Sampler *sampler,
  ...
  while (pos < steps) {
    ...
    if (user_turn) {
      ...
      if (pos == 0 && system_prompt[0] != '\0') {
        char system_template[] =
            ...
            "header_id|>user<|end_header_id|>\n\n%s<|eot_id|><|start_header_id|"
    ...
    if ((token == 128009 || token == 128001) && user_idx == num_prompt_tokens) {
      user_turn = 1;
    }
